...
%\begin{tabular}{@{}lllll@{}}
%\toprule
%     & \multicolumn{2}{l}{Arterial @ 3900 V/h} ($\clubsuit$) & \multicolumn{2}{l}{Grid @ 4800 V/h}  ($\clubsuit$) \\
% & slow l. rail               & fast l. rail               & slow l. rail             & fast l. rail             \\ \midrule
%Fixed   & ~35\%              & ~7.5\%             & ~40\%            & ~17.5\%          \\
%Optimized   & ~2.5\%             & ~(5)\%              & ~10\%            & ~(0.5)\%          \\ \midrule
%Improvement (per vehicle) & 32.5\%          & 12.5\%          & 30\%         & 18\%          \\ \bottomrule
%\end{tabular}
%\phantomcaption \label{fig:table}
%\end{subfigure}
%\vspace{-5mm}
...
Optimized control is able to mitigate the impact of light rail on average delay,
...
the light rail. 
%Table (i): improvement in average delay using optimized control compared to fixed control with the introduction
% of light rail, assuming fixed control was used pre light rail, and evaluated at the demand level corresponding 
 %to points $\clubsuit$ in (a) and $\spadesuit$ in (b). Optimized control is
% able to mitigate the impact to within 10\% of the pre light rail average delay in each case.
%
%% TODO(fwt): add a better summary of the overall results, i.e., add a ``mini''
%% conclusion here
%
%The fixed-adaptive controller has prioritized the arterial phase duration over
...
\remark{Adaptive}    & 97.5\%  & 105.0\%  & 90.0\%  & 100.5\% \\ \hline
...
%
The \remark{optimized} adaptive controller can successfully mitigate the impact of introducing a
...
To further illustrate the advantages of the optimal adaptive controller, we show
...
\remark{fixed-adaptive}\toFelipe{Fixed?} controller has prioritized the arterial phase duration over the
side street duration to achieve coordinated ``green corridors'' sized for the
average traffic density in the network.
%
As byproduct, the side street under the \remark{fixed-adaptive} \toFelipe{Fixed?} controller suffers from
...
%
In particular, for the fast light rail, the \remark{optimized} adaptive controller allows 5\%
