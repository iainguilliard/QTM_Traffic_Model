...
%\includegraphics[width=0.44\textwidth,trim={0cm 0cm 0cm 0cm},clip]{map_example.png}
\hspace{5mm}\includegraphics[width=\textwidth,trim={0cm 0cm 0cm 0cm},clip]{pw_queues_homogenous_v2.pdf}
\end{subfigure}
...
To simplify the presentation of the remainder of the LP, we define
\qin{i}~\eqref{def:qin} and \qout{i}~\eqref{def:qout} to,
...
%find the volume of traffic that entered queue~$i$ between two time points
%$x$ and $y$.
%%
...
%function that returns $j \in \{0,\dots,N\}$ s.t.\ $x$ equals $t_j$ (i.e.,
%the $j$-th time step).
%%
%Since we assumed that \QDELAY{i} is divisible by the different \QDELAY{i},
%$T^{-1}(x)$ is well-defined, i.e., there always exists $j$ such that $x$
%equals $t_j$.
%%
...
the piecewise linear segments of \qin{i} over $[x,y]$ (see \cite{guilliard2016trb}).
%
%%
...
%previous $n - T^{-1}(t_{n}-\QDELAY{i})$ time
%steps, i.e., 
the number of %$\DT[n]$ 
time steps spanned by \QDELAY{i}.
%
...
As with other MILP formulations (e.g., \trbcite{lin2004enhanced}), QTM is also
...
%
QTM addresses this well-known issue by maximizing the external outflow~\outq{i}
%
%(i.e., both internal and external outflow)
%
and external inflow~\inq{i} of all queues~$i$ in its objective
...
Next, the minimum and maximum phase durations (i.e.,
...
inactive; and %~(\ref{c:pd:inactiveUB},\ref{c:pd:inactiveLB}); and
%
(iii) is restarted when phase~$k$ changes from inactive to
active. %~\eqref{c:pd:reset}.
%
%Notice that (\ref{c:pd}) can be encoded by employing the \textit{big-M}
...
%
(\ref{c:pd})~is encoded using the \textit{big-M} method and \PTMAX{\ell}{k} as
...
\pd{\ell}{k}\!&=\!
\begin{cases}
\pd[n\!-\!1]{\ell}{k}\!+\!\DT[n\!-\!1], & \p[n\!-\!1]{\ell}{k}\!= \!\p[n]{\ell}{k}\!=\!1\\
%
0, & \p[n\!-\!1]{\ell}{k}\!=\!0,\p[n]{\ell}{k}\!=\!1\\
%
\pd[n\!-\!1]{\ell}{k}, &\text{otherwise}
%
\end{cases} \tagconstrain{c:pd} \\
 \pd{\ell}{k} &\ge \PTMIN{\ell}{k}(1 - \p{\ell}{k}) \tagconstrain{c:minPhase}
\end{cAlign}
%


Lastly, QTM constrains the sum of all the phase durations for light \tl to be
...
%because \eqref{c:pd} forces \pd[n]{\tl}{1} to be 0 at the beginning of
%each cycle;
%%
...
%\begin{cAlign}
%%
%\textstyle \pd[n\!-\!1]{\ell}{1}\! +\! \sum_{k\!=\!2}^{\Pn} \pd{\ell}{k}\! &\ge\! \CTMIN{\ell}
%(\p[n]{\ell}{1}\! -\! \p[n\!-\!1]{\ell}{1}) \tagconstrain{c:cycleLB}\\
%%
%\textstyle \pd[n\!-\!1]{\ell}{1}\! +\! \sum_{k\!=\!2}^{\Pn} \pd{\ell}{k}\! &\le\! \CTMAX{\ell}
...
\begin{cAlign}
%
 \CTMIN{\ell}(\p[n]{\ell}{1}\! -\! \p[n\!-\!1]{\ell}{1}) \le
%
\pd[n\!-\!1]{\ell}{1}\!+\! \textstyle \sum_{k = 2}^{\Pn} \pd{\ell}{k} \le \CTMAX{\ell}
\tagconstrain{c:cyclelimits}
%
\end{cAlign}
\normalsize
%
...
\begin{cAlign} 
%
\pd{\ell}{k} = \PTFIXED{\ell}{k} \text{ if } \p[n]{\ell}{k}=0 \tagconstrain{c:pd:fixed}
%
\end{cAlign}
%
Similarly to the variable phase duration constraints, the \textit{big-M} method
is employed, using \PTMAX{\ell}{k} as the constant, to enforce (\ref{c:pd:fixed}) only while the
phase is inactive.
%
