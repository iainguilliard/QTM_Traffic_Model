...
convergence point of the Non-homogeneous (NH) and Homogeneous (H) approaches
(\cref{subfig:travel_time_6}).
%
(c)~presents the same curves for the optimal solution.  NH provides
near-optimal signal plans over a longer time horizon than H when the number
of time intervals \Nn is small.}
%
\label{fig:cumu}
\end{figure*}
