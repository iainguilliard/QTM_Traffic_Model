...
\includegraphics[width=0.52\textwidth,trim={1.3cm 1cm 3cm 0.5cm},clip]{map_overlay.pdf}
}
\subfigure[]{
...
\caption{(a) Example of a real traffic network modeled using the QTM. (b) A
  preview of different QTM model parameters as a function of
  \emph{non-homogeneous} discretized time intervals indexed by $n$.
  ...
  duration $d$ of two different light phases for $l_6$, and the phase
  $p$ of light $l_6$ (there is technically a binary $p$ for each
  phase, but here we abuse notation and simply show the current active phase),
  and the traffic volume of different queues $q$ linearly interpolated
  between time points.  Here we can see traffic progress from $q_1$ to $q_7$
  to $q_9$ according to light phases and traffic propagation delay.
...
\item  unlike previous joint intersection control
  work~\trbcitenum{lo1998novel,lin2004enhanced,han2012link}, the QTM is
...
\item any length of roadway with no merges or diverges can be modeled
  ...
  (i.e., large numbers of cells and associated MILP variables are not required \emph{between}
  intersections), and 
%
\item the QTM accurately models fixed travel time delays critical to green wave
  ...
  with the fully joint intersection signal optimization approach
...
%
Next we proceed to allow the traffic signals to
become discrete phase variables that are optimized subject to a delay minimizing 
objective and standard constraints for cycle and phase times; this results in our
