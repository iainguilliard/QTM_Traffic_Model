...
\tn[w+1]$.
%
Because the QTM dynamics is piecewise linear, \qin{i} is a step function
...
function, which has the same effect as minimizing the delay curve.}
...
\remark{\cref{subfig:converg_a,subfig:converg_b} show the results of applying the LP
formulation to a simple model with a fixed signal plan, using both homogeneous
\vecDT and non-homogeneous \vecDT.}
%
\fnremark{This part needs to be connected to the text.}
%
\fnremark{Show diagrams with traffic
predictions converging as time increment gets smaller.  Validates that large
time-steps are rough approximations while model behavior converges for small
time steps.}
