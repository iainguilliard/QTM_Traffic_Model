...
$\TMAX$ & total elapsed time considered (\tn[N])\\[1mm]
\bottomrule\\
\end{tabular}
\end{table}


The Queue Transmission Model (QTM) consists of a network $\Net = (\Qset,\Lset)$,
...
(maximum) allowed cycle time for \tl is represented by \CTMIN{\tl} (\CTMAX{\tl}).
%
For each phase $k \in \Pset_\tl$, \PTMIN{\tl}{k} and \PTMAX{\tl}{k} define the
minimum and maximum allowed time for phase $k$ of
\tl.\remark{\footnote{FWT: \PTMIN{\tl}{k} and \PTMAX{\tl}{k} currently not part
of the tuple}}
%
A queue $i \in \Qset$ represents a segment of road that requires \QDELAY{i}
units of time for vehicles to traverse at free flow speed; once traversed, the
...
%
\QIN{i} (\QOUT{i}) represents the traffic flow from (to) the outside of the
modeled network;
%
...
to $j$ and the turn probability from $i$ to $j$, respectively;
%
and \QPset{i} denotes the set of traffic light phases controlling queue $i$.
%
