...
%%
%% IG: will probably change q_stop to be weighted sum of q_in
%% where weight vector is pre-calcalulated for each n and i.
%%
...
\eqref{c:qUpdate}. If \DT[] is homogeneous for all $n$ and \QDELAY{i}
is a multiple of \DT[] for all $i$, then \eqref{def:qstop}
reduces to $\qstop{i} = \qin[m]{i}$ and \eqref{c:qUpdate}
...
\begin{cAlign}
%
\q{i} &= \q[n-1]{i} - \qout[n-1]{i} + \qstop[n-1]{i}
\tagconstrain{c:qUpdate}\\
%
\frac{\tn - \tn[n+1] + \QDELAY{i}}{\DT[w]}\qin[w]{i}  + \sum \limits_{k=w+1}^n \qin[k]{i}
+ \q[n-1]{i}&\le \QMAX{i} \tagconstrain{c:10}
%
\end{cAlign}
