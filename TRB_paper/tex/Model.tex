...
%
the duration in \remark{seconds} of the $n$-th time interval is denoted as
\DT[n].
%
...
minimum and maximum allowed time for phase $k$ of \tl.\remark{\footnote{FWT:
\PTMIN{\tl}{k} and \PTMAX{\tl}{k} currently not part of the tuple}}
%
A queue~$i \in \Qset$ represents a segment of road that requires \QDELAY{i}
\remark{seconds} for vehicles to traverse at free flow speed; once traversed, the
...
%% TODO(fwt): say that flow to the network from outside is also given
