...
\tn-\QDELAY{i} < \tn[m+1]$. \toFelipe{Perhaps add this: Since the QTM is 
...
\cref{subfig:converg_a,subfig:converg_a,fig:converg_c} show the results of applying the LP formulation to a simple model with a fixed signal plan, using both homogeneous \vecDT and non-homogeneous \vecDT.
...
active~(\ref{c:pd:incUB},\ref{c:pd:incLB}) (see \cref{subfig:test1});
%
(ii) is constant and holds the duration of the last phase until the next
activation when $\p[n]{\ell}{k}$ is
inactive~(\ref{c:pd:inactiveUB},\ref{c:pd:inactiveLB}) (see \cref{subfig:test2}); and
%
(iii) is restarted when phase~$k$ changes from inactive to
active~\eqref{c:pd:reset} (see \cref{subfig:test3}).
%
...
\CTMAX{\tl}~\eqref{c:cycleUB} (see \cref{subfig:test4}).
%
