...
that $\DT[n] = \QDELAY{i}$\remark{\footnote{\remark{$\DT[n] = \QDELAY{i}$}}} for all \mbox{$n \in \{1,\dots,\Nn\}$}, that
...
of \DT[n] that started traversing $i$ before \DT[n-1]. \fnremark{because \DT[n-1] might be smaller than
\QDELAY{i}, therefore, there are cars at the stop line of $i$ in the beginning
...
In order to account for the misalignment \fnremark{misalignment} of the different \DT[] and
...
possible.
%
\eqref{eq:objFunc} is analogous \fnremark{analogous} to minimizing delay in CTM models,
