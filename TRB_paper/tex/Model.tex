...
\begin{itemize}
%
\item $\Pset_\tl$ is the set of phases of $\tl$\revision{, and defines the phase sequence}{4.2};
%
...
queue, allowing arbitrary intersection topologies to be modelled, including ``all red''
 phases as a switching penalty and lost time from yellow lights.}{3.1}
...
\revision{\eqref{c:10} has the effect of forcing \qin[m]{i} to $0$ when queue $i$ is full, 
which forces \f{j}{i} in \eqref{def:qin} and \eqref{def:qout} to 0. This in turn allows  
...
\revision{$\vecDT = \left \{1.0, 1.05, 1.1, 1.16, \ldots , 2.29, 2.41, 2.5 \right \}$}{1.2}
\revout{$\DT[n] \approx 0.0956n + 0.9044$ for $n \in \{1,\dots,17\}$}.  Here we see
