...
\authorHighlight{except for the output queues on Network 1 where the traversal
time is 10s.}\toIain{FIXED: Is this still true?}
%
...
We simulate a fixed phase traffic light controller by employing the QTM to optimize a fixed phase 
duration, $d^{\mathrm{fixed}}_{\ell,k}$, over the planning horizon. 
...
Each network is evaluated at increasing demand levels up to the level where for $\inq{i}$
becomes saturated.
%
...
using the weight functions in \cref{fig:demand_profiles}, where 
$\QIN{i}{n} = \mathrm{max}(\alpha \beta \mathrm{w}_i(\tn[n]),\beta)$, and $\mathrm{w}_i$
is the weight function in \cref{fig:demand_profiles} corresponding to the letter label in 
\cref{fig:networks}, $\beta$ is the saturation flow rate in vehicles per \DT, and $\alpha \in [0,2]$, is the
the weight for the demand level being evaluated.
