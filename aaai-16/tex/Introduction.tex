...
\emph{fixed-time} control~\trbcitenum{el2013multiagent} rather than
\emph{actuated} or \emph{adaptive} control methods such as
SCATS~\trbcitenum{scats80} or SCOOT~\trbcitenum{scoot81}.  As we show
...
\emph{optimized} adaptive controllers as evidenced in a variety of
approaches ranging from mixed integer (linear)
...
learning~\trbcitenum{el2013multiagent}.  To this end, we develop a
linear programming model of traffic flow termed the {\it Queue
  Transmission Model (QTM)} where traffic signals can be represented
as discrete variables and optimized for adaptive control via Mixed
Integer Linear Programming (MILP).  As we show in this paper, such
controllers hold the promise of maximizing existing infrastructure
capacity by finding optimized traffic signal control policies that are
tightly integrated with light rail transit constraints to mitigate
their impact on conventional traffic delays.
