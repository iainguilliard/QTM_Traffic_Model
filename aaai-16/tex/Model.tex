...
\authorHighlight{transit}.

\paragraph{Transit Constraints}
%
To incorporate a fixed-schedule transit in our model, we post-process our MILP
model by fixing the free variable $\p[n]{\tl}{k}$ for all $n$ s.t.\ the transit
uses phase $k$ of \tl at time $n$.
%
Formally, given a transit schedule $S_{\tl}(k,n) \in \{0,1\}$ where~$1$
represents that a transit is using phase $k$ of \tl at time $n$, we replace
...
%To incorporate a fixed-schedule transit in our model, we enforce phase $k$ used
%by the transit is always on when the transit crosses at light \tl, i.e., we add
%the constraint $\p[n]{\ell}{k} = 1$ for all $n$ s.t.\ the transit uses phase $k$
%of \tl at time $n$.
%%
%%
%We also hold $\pd{\ell}{k}$ constant while the transit crosses \tl, i.e., 
%$\pd{\ell}{k} = \pd[n-1]{\ell}{k}$.
%%
%Since the transit schedule is fixed and known a priori, adding these extra
