...
\includegraphics[width=0.45\textwidth,trim={1.3cm 1.1cm 3cm 0.5cm},clip]{map_overlay.pdf}}
\subfigure[]{
\label{subfig:example}
%\includegraphics[width=0.45\textwidth,trim={0cm 0cm 0cm 0cm},clip]{map_example.png}
\includegraphics[width=0.45\textwidth,trim={0cm 0cm 0cm 0cm},clip]{pw_queues_homogenous.pdf}
}
...
\caption{(a) Example of a real traffic network modeled using the
  ...
  \authorHighlight{FWT: this figure is not being referenced yet}.}
\label{fig:qtm}
%
\end{figure}
...
%
\authorHighlight{Instead, QTM assumes that \QDELAY{i} is divisible by every
different value of $\DT[n]$.
%
...
because the update considers the previous $n - T^{-1}(t_{n-1}-\QDELAY{i})$ time
steps.
%
...
In this section, we show how to compute the \authorHighlight{optimized adaptive}
...
the problem of finding the \authorHighlight{optimized adaptive} traffic control
